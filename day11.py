...
class AllEnergizedException(Exception): pass
...
def initialize_matrix(dataset):
    ...
    width = len(matrix[0])
    height = len(matrix)
    new_matrix = [[None for _ in range(width+2)] for __ in range(height+2)]
...
def single_step(matrix):
    ...
    def get_high_energy():
        ...
        return any(any(x[0] > 9 and x[1] == False for x in row) for row in center_matrix)
    
    def is_all_energized():
        center_matrix = get_center_matrix(matrix)
        return all(all(x[1] == True for x in row) for row in center_matrix)
    ...
    while get_high_energy():
        for row, col in itertools.product(range(1, matrix_h - 1), range(1, matrix_w - 1)):
            ...
            if matrix[row][col][0] > 9 and matrix[row][col][1] == False:
