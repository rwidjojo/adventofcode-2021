...
def count_occurrence(matrix, value):
    filtered = [list(filter(lambda x: x == value, row)) for row in matrix]
...
basin_sizes = [(low_point[0], count_occurrence(new_matrix, names[low_point[0]])) for low_point in low_points]
