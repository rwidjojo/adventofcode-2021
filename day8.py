...
def count_occurence(list_of_signals, idx):
    result = [list(filter(lambda x: len(x) == idx, row)) for row in list_of_signals]
...
def generate_final_mapping(original_input):
    ...
    unique_signal_dict = {letter: unique_signal_input.count(letter) for letter in set(list(unique_signal_input))}
